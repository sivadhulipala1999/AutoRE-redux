...
# Test user input
# deepspeed --master_port 12347 --include localhost inference.py --model_name_or_path ${BASE_MODEL} --adapter_name_or_path ${model_path} --template ${model} --data_path ${DATA_PATH} --version ${version} --do_sample true --max_new_tokens 1024 --temperature 0.95 --top_p 0.6 --lora_test ${lora_test} --relation_step ${relation_step} --subject_step ${subject_step} --fact_step ${fact_step} --save_path ${EVAL_SAVE_PATH}
deepspeed --master_port 12347 --include localhost inference.py --model_name_or_path lmsys/vicuna-7b-v1.5 --adapter_name_or_path ${model_path} --template ${model} --data_path ${DATA_PATH} --version ${version} --do_sample true --max_new_tokens 1024 --temperature 0.95 --top_p 0.6 --lora_test ${lora_test} --save_path ${EVAL_SAVE_PATH}
